...
trait FlowExecutionObserver[F[_]] {
  ...
  def gatewayFinished(gateway: ExecutingOperator[GatewayOp]): F[Unit]
  ...
  def ruleFinished(rule: ExecutingOperator[RuleOp]): F[Unit]

  def tableStarted(table: ExecutingOperator[TableOp]): F[TableOp]
  def tableFinished(table: ExecutingOperator[TableOp], executedRows: Int): F[Unit]
...
class NoOpFlowExecutionObserver[F[_]](implicit monadError: MonadError[F, NdkError]) extends FlowExecutionObserver[F] {
  ...
  override def pyOperatorStarted(po: ExecutingOperator[PythonOperatorOp[Any, Any]]): F[PythonOperatorOp[Any, Any]] = po.op.pure
